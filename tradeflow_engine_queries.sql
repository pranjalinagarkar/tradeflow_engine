...
--Lets add one more table for market prices.

CREATE OR REPLACE TABLE dbt.raw.market_price_load (
    market_date DATE,
    instrument_id VARCHAR,
    close_price NUMBER(18,4)
);

CREATE OR REPLACE STAGE TRADEFLOWENGINE_SOURCE_AWS_PRICES_STAGE
FILE_FORMAT='CSVTYPE'
STORAGE_INTEGRATION = TRADEFLOWENGINE_AWS_STORAGE_INTEGRATION
URL='s3://tradeflow-engine-raw/prices/'; 

CREATE OR REPLACE PIPE price_snowpipe 
  AUTO_INGEST = TRUE 
AS
  COPY INTO dbt.raw.market_price_load
  FROM @TRADEFLOWENGINE_SOURCE_AWS_PRICES_STAGE
  ON_ERROR = 'CONTINUE';

select * 
from dbt.raw.market_price_load;

describe pipe price_snowpipe;
